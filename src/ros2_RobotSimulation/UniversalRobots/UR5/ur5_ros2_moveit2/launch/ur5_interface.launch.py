...
def generate_launch_description():
    ...
    ur5_ros2_gazebo = os.path.join(
        get_package_share_directory('ros2_hri'),
        'world',
        'Scene10.world')
    ...
    # ***** ROBOT DESCRIPTION ***** #
        # ***** Adicionar definições das câmeras ***** #
    urdf_camera_files = [
        'urdf/urdf.urdf', 
        'urdf/urdf2.urdf', 
        'urdf/urdf3.urdf', 
    ...
    spawn_camera_processes = []

    for idx, urdf_camera_file in enumerate(urdf_camera_files):
        urdf_camera_path = os.path.join(get_package_share_directory("ros2_hri"), urdf_camera_file)
        xml_camera = open(urdf_camera_path, 'r').read()
        xml_camera = xml_camera.replace('"', '\\"')
        spawn_args_camera = '{name: "camera' + str(idx+1) + '", xml: "' + xml_camera + '"}'
        
        spawn_process_camera = ExecuteProcess(
            cmd=['ros2', 'service', 'call', '/spawn_entity',
                 'gazebo_msgs/SpawnEntity', spawn_args_camera],
            output='screen'
        )
        
        spawn_camera_processes.append(spawn_process_camera)
